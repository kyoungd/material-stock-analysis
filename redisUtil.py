...
class RetentionTime(Enum):
    SECOND = 1000
    MINUTE = 60000
    HOUR = 360000


class AlpacaAccess(Enum):
...
def bar_key(symbol, suffix, timeframe):
    return "data_" + suffix + "_" + timeframe.value + ":" + symbol
...
class KeyName(Enum):
    ...
    def KeyBar(symbol, datatype, timeframe):
        return "RTS_BAR_" + datatype.upper() + "_" + timeframe.value + ":" + symbol.upper()
...
class TimeStamp:
    ...
    def get_starttime(timeframe):
        second = RetentionTime.SECOND.value
        minute = 60 * second
        hour = 60 * minute
